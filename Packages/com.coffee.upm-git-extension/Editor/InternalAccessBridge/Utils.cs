...
namespace UnityEditor.PackageManager.UI.InternalAccessBridge
{
	...
	internal static class GitUtils
	{
		...
		public static void GetRefs (string repoUrl, Action<IEnumerable<string>> callback)
		{
			...
			string escapedUrl = Uri.EscapeDataString (repoUrl);
			if (!Directory.Exists(escapedUrl))
			{
				//Directory.CreateDirectory (escapedUrl);
				//クローンを実行
				ExecuteGitCommand (string.Format("clone -q --no-checkout --depth=1 {0} {1}", repoUrl, escapedUrl), (success, output) =>
				{
					if (success)
					{
						Debug.Log (output);
			...
			{
				// フェッチを実行
				ExecuteGitCommand ("fetch --depth=1 -fq --prune origin 'refs/tags/*:refs/tags/*' '+refs/heads/*:refs/remotes/origin/*'", (success, output) =>
				{
					if (success)
					{
						Debug.Log (output);

						ExecuteGitCommand ("git show-ref", (success2, output2) =>
						{
							if (success2)
							{
								Debug.Log (output2);
			...
			command.AppendFormat ("cd {0} && ", escapedUrl);
			command.AppendFormat ("git fetch --depth=1 -fq --prune origin 'refs/tags/*:refs/tags/*' '+refs/heads/*:refs/remotes/origin/*' && ");
			command.AppendFormat ("git show-ref");

			//git show-ref // <- get all branche


			ExecuteCommand(command.ToString(), (success, output) =>
			{
				if (success)
				{
					Debug.Log (output);
		...
		static void ExecuteGitCommand (string args, GitCommandCallback callback, string cwd = null)
		{
